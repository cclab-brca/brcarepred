...
dim(na.omit(Clinical[,c('Pam50Subtype', 'iC10', 'Group', 'Size', 'Grade', 'Lymph.Nodes.Positive', 'T', 'LR', 'DR', 'TLR', 'TDR', 'Death', 'DeathBreast')]))

## Transition matrix
tra <- matrix(NA, 5*4, 5*4)

state.names <- paste(c("PostSurgery",
                            "LocoregionalRelapse", "DistantRelapse",
                       "CancerDeath", "NaturalDeath"),
                     rep(levels(Clinical$Group), rep(5, 4)), sep="_")
rownames(tra) <- state.names
colnames(tra) <- rownames(tra)
counter <- 1
for (i in seq(from=1, by=5, length=4)) {
...
fm <- coxph(Surv(time, status) ~ strata(trans)  + AGE.PS + AGE.LR + AGE.DR + GRADE.PS + GRADE.LR + GRADE.DR + SIZE.PS + SIZE.LR + SIZE.DR +
                LN.PS + LN.LR + LN.DR +
                    TLastSurgery.LR + TLastSurgery.DR + cluster(id), data=res)


m0 <- coxph(Surv(time, status) ~ strata(trans)  + AGE.PS.LR + AGE.DR + SIZE + GRADE + LN + TLastSurgery, data=res)
...
ids <- which(Clinical$Group=="ER+/HER2+")
...
ids <- which(Clinical$Group=="ER-/HER2+")
newdata <- rbind(newdata, data.frame(strata=19:27,
                      AGE=mean(Clinical$AGE[ids], na.rm=T),
                      LN=mean(Clinical$LN[ids], na.rm=T),
                      GRADE=mean(Clinical$GRADE[ids], na.rm=T),
                      SIZE=mean(Clinical$SIZE[ids], na.rm=T),
...
ids <- which(Clinical$Group=="ER-/HER2-")
newdata <- rbind(newdata, data.frame(strata=28:36,
                      AGE=mean(Clinical$AGE[ids], na.rm=T),
                      LN=mean(Clinical$LN[ids], na.rm=T),
                      GRADE=mean(Clinical$GRADE[ids], na.rm=T),
                      SIZE=mean(Clinical$SIZE[ids], na.rm=T),
                 ...
                 rel.can <- c(1:4, 7, 9, 10:13, 16, 18, 19:22, 25, 27, 28:31, 34, 36)
...
save(m, newdata, Clinical, tra, res, cens, file="../Models/FourGroupsM.RData")
