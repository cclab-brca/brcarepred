...
pdf("FigureS12.pdf", width=12, height=12)
par(mfrow=c(1,2))
...
text(-0.5, -1, "Other", cex=size.text, adj=0.5)
...
lines.end.y <- c(-0.5, 0.95,
                 ...
                 -0.9,
                 0.1, -0.1, -0.2)
bar.cols <- rep("white", 22)
bar.cols[seq(from=1, by=2, length=11)] <- coliClust
for (i in 1:length(all.sites)) {
    ...
    feo <- barplot(cbind(prop.table(res[[i]], 2)[,1,drop=F], matrix(rep(NA, 2 * (11-1)), nrow=2)), axes=F, names.arg=rep("", 11),
             col=c(coliClust[1], "white"), cex.axis=1.5, cex.lab=1.5, cex.names=1.5)
    for (j in 2:11) {
        barplot(cbind(matrix(rep(NA, 2 * (j-1)), nrow=2), prop.table(res[[i]], 2)[,j,drop=F], matrix(rep(NA, 2 * (11-j)), nrow=2)), axes=F, names.arg=rep("", 11),
             col=c(coliClust[j], "white"), add=T)
    }
    if (i==4) mtext(at=feo, side=3, text=c(1:3, "4+", "4-", 5:10),
            cex=1, las=2, line=1)
    if (any(all.pvals[[i]] < 0.05)) {
        pch <- ifelse(all.coefs[[i]][all.pvals[[i]]<0.05]>0, 24, 25)
        points(feo[which(all.pvals[[i]]<0.05)],
               1.1, pch=pch, col="black", bg="black", cex=1)
    }
    ids <- seq(from=1, by=2, length=6)
    axis(1, at=feo[ids], res[[i]]['1',][ids], line=-1, cex.axis=0.7, tick=FALSE)
    ids <- seq(from=2, by=2, length=5)
    axis(1, at=feo[ids], res[[i]]['1',][ids], line=-0.5,
...
m <- list()
for (i in c(1:3, "4ER+", "4ER-", 5:10)) {
m[[i]] <- survfit(Surv(time, status) ~ strata(SITE), data=CR, subset=iC10==i)
}
