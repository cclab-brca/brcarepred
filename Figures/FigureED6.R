...
ER.mean <- lapply(boot.TP, function(x) lapply(x, function(y) apply(y, 1, mean, na.rm=T)))
ER.sd <- lapply(boot.TP, function(x) lapply(x, function(y) apply(y, 1, sd, na.rm=T)))

names(ER.sd) <- names(pt)


cols <- c("lightblue", "darkblue", "red", "olivedrab")
cols <- rep(cols, 2)
...
for (k in 6:9) {
    lines(times, pt$POS[[k]], lwd=2, col=cols[k-5], cex.axis=3)
    polygon(c(times, rev(times), times[1]),
            c(pt$POS[[k]]- 1.96*ER.sd$POS[[k]],
              rev(pt$POS[[k]]+ 1.96*ER.sd$POS[[k]]),
              pt$POS[[k]][1]- 1.96*ER.sd$POS[[k]][1]),
            col=adjustcolor(cols[k-5], alpha=0.2), border=NA)
    }
plot(times, rep(0, length(times)), type="n",
     ylim=c(0,1), xlim=c(0, 20), xlab="", ylab="")
title(main="ER-", line=1)
for (k in 6:9) {
    lines(times, pt$NEG[[k]], lwd=2, col=cols[k-5])
    polygon(c(times, rev(times), times[1]),
...
BOOT.mean <- lapply(boot.TP, function(x) lapply(x, function(y) apply(y, 1, mean, na.rm=T)))
BOOT.sd <- lapply(boot.TP, function(x) lapply(x, function(y) apply(y, 1, sd, na.rm=T)))
...
for (k in 6:9) {
    lines(times, pt[[id]][[k]], lwd=2, col=cols[k-5], cex.axis=3)
    polygon(c(times, rev(times), times[1]),
...
legend("left", fill=cols[1:5], legend=c("LR", "DR", "D/C", "D/O"), bty="n", ncol=2,
       cex=1.2)
mtext("Years after loco-regional relapse",1,outer=TRUE, at=0.5, cex=1.3)
